  ...
  <div class="max-w-2xl mx-auto p-6 bg-stone-50 min-h-screen">
    ...
    <div class="space-y-4 mb-4">
      <div v-for="item in basketItems" :key="item.id"
        ...
        <div
          class="flex flex-row sm:flex-col items-center sm:items-end justify-between sm:justify-center w-full sm:w-auto mt-4 sm:mt-0 gap-3 border-t sm:border-t-0 pt-3 sm:pt-0 border-stone-100">
    ...
    <div v-if="basketItems.length === 0"
      class="text-center py-12 bg-white rounded-2xl border border-stone-100 shadow-sm my-6">
      <svg xmlns="http://www.w3.org/2000/svg" class="h-12 w-12 mx-auto text-stone-300 mb-3" fill="none"
    ...
    <div class="bg-stone-100 rounded-xl overflow-hidden transition-all duration-200">
      <button @click="isInstructionsOpen = !isInstructionsOpen"
        ...
        <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 transform transition-transform duration-200"
          :class="{ 'rotate-180': isInstructionsOpen }" fill="none" viewBox="0 0 24 24" stroke="currentColor">
          <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7" />
        </svg>
      </button>

      <div v-show="isInstructionsOpen" class="p-4 pt-0 bg-stone-100">
        <textarea v-model="specialInstructions"
          placeholder="E.g., Allergies, delivery preferences, or burger temperature details..."
          class="w-full p-3 text-sm bg-white border border-stone-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-stone-400 placeholder-stone-400 resize-none h-24"></textarea>
      </div>
    </div>
  </div>
</template>

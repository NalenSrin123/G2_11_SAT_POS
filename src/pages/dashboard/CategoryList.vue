    ...
    <div class="flex h-screen bg-slate-50 font-sans">
      ...
      <main class="flex-1 overflow-auto px-5 py-6 max-w-5xl mx-auto">
        ...
        >
          <!-- Toolbar -->
          <div
            class="flex flex-wrap items-center gap-3 px-5 py-4 border-b border-slate-100 bg-slate-50"
          >
            <!-- Status filter -->
            <div class="relative">
              <svg
                class="w-3.5 h-3.5 text-slate-400 absolute left-3 top-1/2 -translate-y-1/2 pointer-events-none"
                fill="none"
                stroke="currentColor"
                viewBox="0 0 24 24"
              >
                <path
                  ...
                  stroke-linejoin="round"
                  stroke-width="2"
                  d="M3 4h18v2.172a2 2 0 01-.586 1.414L13 14.414V21l-6-3v-3.586L3.586 7.586A2 2 0 013 6.172V4z"
                />
              </svg>
              <select
            ...
            <table class="w-full">
              <thead>
                <tr class="border-b border-slate-100">
                  <th
                    class="px-5 py-3 text-left text-[11px] font-bold uppercase tracking-widest text-slate-400"
                  >
                    Category Name
                  </th>
                  <th
                    class="px-5 py-3 text-left text-[11px] font-bold uppercase tracking-widest text-slate-400 hidden md:table-cell"
                  >
                    Description
                  </th>
                  <th
                    class="px-5 py-3 text-center text-[11px] font-bold uppercase tracking-widest text-slate-400"
                  >
                    Items
                  </th>
                  <th
              ...
              <tbody>
                ...
                >
                  ...
                  <td class="px-5 py-3.5 hidden md:table-cell">
                    <span
                      class="text-slate-500 text-sm line-clamp-1 max-w-xs block"
          ...
          >
            ...
            <div class="flex items-center gap-1">
              <template v-for="p in visiblePages" :key="'page-' + p">
                <span
                  v-if="p === '...'"
                  class="px-1 text-slate-300 text-sm select-none"
                  >…</span
                >
                <button
                  v-else
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
          <!-- Management Insights -->
          <div
            class="relative rounded-2xl overflow-hidden min-h-[160px] bg-slate-800 group cursor-pointer"
          >
            <img
            ...
            <div class="absolute bottom-0 left-0 p-5 z-20">
              ...
              <p class="text-slate-300 text-xs mt-1">
                Review category performance metrics and seasonal trends.
              </p>
            </div>
            <div
          ...
          <!-- Smart Sorting -->
          <div
            class="rounded-2xl p-5 min-h-[160px] flex flex-col justify-between bg-blue-500"
          >
            <div>
              <h3 class="text-white font-bold text-lg">Smart Sorting</h3>
              <p class="text-blue-200 text-xs mt-1 leading-relaxed">
                Auto-categorize items based on AI-driven tags.
              </p>
            </div>
            <button
        ...
        <!-- footer -->
        <footer
          class="text-center text-xs text-slate-full py-5 border-t border-slate-200 bg-white w-full"
        >
          © 2024 Workspace Admin. All rights reserved. Precision Built
          Management.
        </footer>
      </main>
        ...
        >
          <div class="bg-white rounded-2xl shadow-2xl w-full max-w-md p-7">
            ...
            <div class="space-y-4">
              ...
              <div>
                <label
                  class="block text-[11px] font-bold uppercase tracking-widest text-slate-400 mb-1.5"
                >
            ...
            <!-- Error -->
            <p v-if="formError" class="text-red-500 text-xs font-semibold mt-3">
              {{ formError }}
            </p>
            <!-- Buttons -->
...
export default {
  ...
  computed: {
    filteredCategories() {
      let arr = this.categories.filter(
        (c) => !this.filterStatus || c.status === this.filterStatus,
      );
      if (this.sortKey === "name")
        arr = [...arr].sort((a, b) => a.name.localeCompare(b.name));
      if (this.sortKey === "items")
        arr = [...arr].sort((a, b) => b.items - a.items);
      if (this.sortKey === "status")
        arr = [...arr].sort((a, b) => a.status.localeCompare(b.status));
      return arr;
    },
    totalPages() {
      return Math.max(
        1,
        Math.ceil(this.filteredCategories.length / this.perPage),
      );
    },
    pageStart() {
    ...
    visiblePages() {
      ...
      const pageSet = new Set(
        [1, t, c, c - 1, c + 1].filter((x) => x >= 1 && x <= t),
      );
